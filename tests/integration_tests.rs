...
fn test_ignore_gitignore() {
    ...
    fs::write(test_dir.join("nested_include").join("included2.txt"), "This nested file should be included").unwrap();
    
    fs::write(test_dir.join("nested_ignore").join(".gitignore"), "nested_ignore.txt").unwrap();
    fs::write(test_dir.join("nested_ignore").join("nested_ignore.txt"), "This nested file should not be included").unwrap();
    fs::write(test_dir.join("nested_ignore").join("actually_include.txt"), "This nested file should actually be included").unwrap();
    ...
    let expected_included = test_dir.join("included.txt").to_string_lossy().to_string().to_string();
    let expected_nested_included = test_dir.join("nested_include/included2.txt").to_string_lossy().to_string().to_string();
    let expected_actually_include = test_dir.join("nested_ignore/actually_include.txt").to_string_lossy().to_string().to_string();
    let expected_ignored = test_dir.join("ignored.txt").to_string_lossy().to_string().to_string();
    ...
    let expected_ignored = test_dir.join("ignored.txt").to_string_lossy().to_string().to_string();
    let expected_nested_included = test_dir.join("nested_include/included2.txt").to_string_lossy().to_string().to_string();
    let expected_nested_ignored = test_dir.join("nested_ignore/nested_ignore.txt").to_string_lossy().to_string().to_string();
    let expected_actually_include = test_dir.join("nested_ignore/actually_include.txt").to_string_lossy().to_string().to_string();
    assert!(filenames.contains(&expected_included));
    assert!(filenames.contains(&expected_ignored));
    assert!(filenames.contains(&expected_nested_included));
...
fn test_multiple_paths() {
    ...
    let expected_single = temp_dir.path().join("single_file.txt").to_string_lossy().to_string();
...
fn test_ignore_patterns() {
    ...
    let stdout = String::from_utf8(output).unwrap();
    let expected_include = test_dir.join("file_to_include.txt").to_string_lossy().to_string();
...
fn test_markdown() {
    ...
    let expected_quad_backticks = test_dir.join("python_with_quad_backticks.py").to_string_lossy().to_string();
